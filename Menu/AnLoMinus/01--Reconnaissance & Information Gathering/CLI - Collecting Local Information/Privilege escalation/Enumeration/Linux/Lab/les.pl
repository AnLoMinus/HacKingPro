...
sub get_kernel {
    ...
    if (!defined $khost || !($khost =~ /^[0-9]+([.][0-9]+)*$/)) {
        print "  Specified kernel is in the wrong format\n";
        print "  Try a kernel format like this: 2.3.45\n";
        exit;
    }
...
sub download_exploits {
    ...
    foreach my $num (@selected) {
        ...
        if ($exploit_num && index($mil, $exploit_base) != -1) {
            ...
            print "  Can't download exploit for \e[00;33m$applicable[$num-1]{key}\e[00m\n"; 
        }
    }
}
...
sub usage {
print_banner();
print "\n  \e[00;35mUsage:\e[00m $0 [-h] [-k kernel] [-d]\n\n";
